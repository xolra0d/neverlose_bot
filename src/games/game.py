...
class Game(ABC):
    ...
    @abstractmethod
    def initial_state(self) -> tp.Any:
        """Returns starting state"""
        pass

    @abstractmethod
    def get_legal_moves(self, state: tp.Any) -> tp.List[tp.Any]:
        """Returns list of valid moves from this position"""
        pass

    @abstractmethod
    def add_move(self, state: tp.Any, move: tp.Any) -> tp.Any:
        """Returns new position after making move"""
        pass

    @abstractmethod
    def generate_best_move(self, state: tp.Any) -> tp.Any:
        """Returns best move from this position"""
        pass

    @abstractmethod
    def is_terminal(self, state: tp.Any) -> bool:
        """Check if game is over"""
        pass

    @abstractmethod
    def get_winner(self, state: tp.Any) -> tp.Optional[int]:
        """
    ...
    @abstractmethod
    def format_state(self, state: tp.Any) -> str:
        """Format game state for display to user"""
        pass

    @abstractmethod
    def parse_move(self, move_str: str) -> tp.Optional[tp.Any]:
        """
